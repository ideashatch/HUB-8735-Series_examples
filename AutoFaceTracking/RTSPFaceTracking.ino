...
#define EN_AP_MODE  false

char ssid[] = "WINSTEC_5G";    // Set the SSID
char pass[] = "2a43b09ff7";        // Set the password
char apssid[] = "AP_Network_SSID";    // Set the AP SSID
char appass[] = "12345678";        // Set the AP password

int status = WL_IDLE_STATUS;
char channel[] = "1"; 
int ssid_status = 0;   
bool showRTSPURL = true;
...
void loop() {
    ...
    if (Serial.available() > 0) {
        ...
        } else if (input.startsWith(String("ANGLE=")))
        {
            String sangle = input.substring(6);
            int iangle = sangle.toInt();
            if ((iangle >= 0) && (iangle <= myservo_MAX))
            {
              myservo.write(iangle);
              current_angle = iangle;
            }
            else
            {
              myservo.write(0);
        ...
        else if (input.startsWith(String("TRACKING=")))
        {
            Tracking_name = input.substring(9);
        }
    }
  ...
  if (showRTSPURL)
  {
    ...
    if (facerecog.getResultCount() > 0) {
        for (uint32_t i = 0; i < facerecog.getResultCount(); i++) {
            ...
            if (String(item.name()) == Tracking_name)
            {
                LockFocus = true;
                osd_color = OSD_COLOR_WHITE;
            }
            else
            {
                LockFocus = false;
            ...
            if (LockFocus)
            {
              ...
              if (count > 10)
              {
                  if ((xmin < 780) && (current_angle < myservo_MAX ))
                  {
                      offset_angle=ceil((780-xmin)/tran_pixel);
                      
                      if (current_angle + offset_angle > 180)
                      {
                          offset_angle = 0;
                          current_angle = 180;
                      }
                      ...
                      //printf("current_angle l  %d\n\r", current_angle);
                  }
                  else if ((xmax > 1140) && (current_angle >= 0))
                  {
                      offset_angle= ceil((xmax-1140)/tran_pixel);

                      if (current_angle - offset_angle < 0)
                      {
                          offset_angle = 0;
                          current_angle = 0;
                      }
                      ...
                      //printf("current_angle r %d %d\n\r",offset_angle, current_angle);
                  }
                  else
                  {
                      offset_angle = 0;
                      //printf("middle or else %d\n\r", current_angle);
                  }
            ...
            float fdistance = 18600.00/(float)(xmax-xmin)/100.00;
            // Print identification text above boundary box
            char text_str[40];
            snprintf(text_str, sizeof(text_str), "%s %.1fm", item.name(), fdistance);
